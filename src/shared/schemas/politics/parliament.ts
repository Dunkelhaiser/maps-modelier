...
export const parliamentSchema = zod.object({
    ...
    oppositionLeader: zod.coerce.number().int({ message: "Select opposition leader" }).optional(),
    coalitionLeader: zod.coerce.number().int({ message: "Select coalition leader" }).optional(),
});
