...
import { useAssignHeadOfGovernment, useGetHeadOfGovernment } from "@ipc/government";
...
const HeadOfGovernmentForm = () => {
    ...
    const defaultValues = useMemo(
        () => ({
            head: headOfGovernment?.id,
            title: headOfGovernment?.title,
            startDate: (formatLocalDate(headOfGovernment?.startDate) as unknown as Date | null) ?? undefined,
            endDate: (formatLocalDate(headOfGovernment?.endDate) as unknown as Date | null) ?? undefined,
        }),
        [headOfGovernment?.endDate, headOfGovernment?.id, headOfGovernment?.startDate, headOfGovernment?.title]
    );
    ...
    const assignHeadOfGovernment = useAssignHeadOfGovernment(activeMap, selectedCountry);

    const assignHeadOfGovernmentHandler = async (data: AssignHeadInput) => {
        await assignHeadOfGovernment.mutateAsync(data);
    };

    return (
        <div className="flex flex-col gap-4">
            <h3 className="text-lg font-medium">Head of Government</h3>
            <Form {...form}>
                <form className="flex flex-col gap-4 py-4" onSubmit={form.handleSubmit(assignHeadOfGovernmentHandler)}>
                    <FormField
...
export default HeadOfGovernmentForm;
