...
import { useAddMembers } from "@ipc/parties";
...
import { AddMembersFormInput, addMembersFormSchema, AddMembersInput } from "src/shared/schemas/parties/addMembers";
...
const AddMembersForm = ({ members, leaderId }: Props) => {
    ...
    const addMembers = useAddMembers(activeMap, selectedParty);

    const defaultValues = useMemo(
        () => ({
            members: members?.map((member) => ({ politicianId: member.id })) ?? [],
        }),
        [members]
    );
    ...
    const addMembersHandler = async (data: AddMembersFormInput) => {
        await addMembers.mutateAsync(data.members.map((m) => m.politicianId) as AddMembersInput);
    };

    const selectedMembers = form.watch("members").map((m) => Number(m.politicianId));

    return (
        <Form {...form}>
            <form className="grid gap-4 pt-4" onSubmit={form.handleSubmit(addMembersHandler)}>
                <Label>Prominent Members</Label>
                        ...
                        {membersField.map((memberField, index) => {
                            ...
                            return (
                                <div className="flex gap-2" key={memberField.id}>
                                    <FormField
                                        control={form.control}
                                        name={`members.${index}.politicianId`}
                                        render={({ field }) => (
                                            <FormItem>
                                                <FormControl>
                                                    <MembersSelect
                                                        ...
                                                        isLeader={memberField.politicianId === leaderId}
                                                    />
                                                </FormControl>
                                                <FormMessage />
                                            </FormItem>
                                    ...
                                    />
                                    {memberField.politicianId !== leaderId && (
                                        <Button
                                            variant="ghost"
                                            aria-label="Remove"
                                            size="icon"
                ...
                <div className="flex gap-2">
                    <Button type="button" onClick={() => append({ politicianId: -1 })} className="flex-1 gap-2">
                        <Plus />
                        Add Member
                    </Button>
